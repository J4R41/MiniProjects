...
let Sn = []; //doda [n,x,y,width,height,text,color]
...
function test()
{
	...
	main();
	type_sections();
	document.getElementById('text_into_sn').value = 'Panatol extra forte gdy brakuje ci atencji';
	document.getElementById('s_n').value = 5;
	document.getElementById('colorid').value = '#00ff88';
...
function main()
{
	...
	document.getElementById('sectionst').value = 'Table changed';
}
...
function type_sections()
{
	...
	else
	{
		...
		while(y < maxy)
		{
			while(x < maxx)
			{
				if((find_in_t(x,'xt') - x - 1) > 0 && (find_in_t(y,'yt') - y - 1) > 0)
				{
					n_of_S++;
					Sn.push([n_of_S,(x + 1),(y + 1),(find_in_t(x,'xt') - x - 1),(find_in_t(y,'yt') - y - 1),'','#8800ff']);
				}
...
function exprt()
{
	let widthbg = 7.2 * maxx;
	document.getElementById('dataexport').value = '<div style="background-color:black; color:#8800ff; font-family: monospace; width: ' + widthbg + 'px;">' + Data.map(e => e.join('')).join('<br>') + '<div>';
}
function imprt()
{
